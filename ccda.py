...
class Root(object):
  ...
  ALLERGY_RECATION = '2.16.840.1.113883.10.20.22.4.9'
...
''' Important class to work and modify'''
class CcdaDocument(object):
  ...
  def to_message(self):
    ...
    doc.allergies = []  # Create a list to store details of allergies from given C-CDA Xml file.
    ...
    for entry in entries:
        allergy = messages.Allergy()  # For each DOM, use messages.py to create an object of class Allergy to store
        # information about an allergy.

        ''' Enter allergy observation details '''

        date_node = entry.getElementsByTagName('effectiveTime')[0]  # date_node is first DOM for tag <effectiveTime>.
        ...
        if participant:
            participantRole = participant.getElementsByTagName('participantRole')[0]
            ''' # [0] for first participantRole object from a list of objects for <participantRole> under <participant>  '''
            if participantRole:
                playingEntity = participantRole.getElementsByTagName('playingEntity')[0]
        ...
        allergy_status = self._tree.get_entries_by_template(Root.ALLERGY_STATUS, parent=entry)[0]
        allergy_status_value_node = allergy_status.getElementsByTagName('value')[0]
        allergy_status_value = CcdaTree.get_code_from_node(allergy_status_value_node)
        allergy.allergen = messages.Code(**allergy_status_value)

        ''' Enter allergy reaction details '''
        allergy_reaction = self._tree.get_entries_by_template(Root.ALLERGY_RECATION, parent=entry)[0]
        allergy_reaction_value_node = allergy_reaction.getElementsByTagName('value')[0]
        allergy_reaction_value = CcdaTree.get_code_from_node(allergy_reaction_value_node)
        allergy.reaction = messages.Code(**allergy_reaction_value)

        ''' Enter allergy severity details '''
        allergy_severity = self._tree.get_entries_by_template(Root.ALLERGY_SEVERITY, parent=entry)[0]
        allergy_severity_value_node = allergy_severity.getElementsByTagName('value')[0]
        allergy.severity = allergy_severity_value_node.getAttribute('displayName')
